...
export async function createHiredFixture(database: PostgresTestDatabase, suffix: string) {
  ...
  const candidateId = String(application.rows[0].candidate_id);
  await request(app)
    .post(
    ...
    .expect(200);
  return { app, ...fixture, applicationId, candidateId };
}

export async function createActiveApplicationFixture(
  ...
  return {
    ...
    candidateId: String(application.rows[0].candidate_id)
  };
}
...
export function createRecruitmentEmployment(
...
export function employmentAction(
  fixture: Awaited<ReturnType<typeof createHiredFixture>>,
  employmentId: string,
  action: "activate" | "cancel" | "end",
  key: string = crypto.randomUUID(),
  body: Record<string, unknown> = {}
) {
  return request(fixture.app)
    .post(`/api/organizations/${fixture.organizationId}/employments/${employmentId}/${action}`)
    .set(userHeaders(fixture.ownerId))
    .set("Idempotency-Key", key)
    .send(body);
}
